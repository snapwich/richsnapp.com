<template>
  <div>
    <div
      class="btn-group btn-group-justified form-group social-links visible-xs visible-sm"
      role="group"
    >
      <a
    ...
    <p>
      Feel free to contact by clicking the social icons above or using the form
      below. Be sure to include a return email address or some form of contact
      information in the message body if you'd appreciate a response.
    </p>
    <form name="contact" novalidate class="contact-form" v-on:submit.prevent>
      <div v-if="!submitted">
        <div class="row">
          <div
            ...
            :class="{ 'has-error': submitAttempted && errors.has('name') }"
          >
          ...
          <div
            class="form-group col-sm-6"
            :class="{ 'has-error': submitAttempted && errors.has('email') }"
          >
        ...
        <div
          class="form-group"
          :class="{ 'has-error': submitAttempted && errors.has('message') }"
        >
        ...
        <div class="row">
          <div
            class="form-group form-group col-sm-8"
          ...
          <div class="form-group col-sm-4">
            <vue-recaptcha
              ref="captcha"
              @verify="onVerify"
              sitekey="6LdL9T4UAAAAALj9ztC5O1Unlh1Y-Mf8OjFBUpo0"
            >
      ...
      <div
        ...
        v-if="submitAttempted && errors.any()"
        role="alert"
      >
    ...
    onVerify(recaptcha) {
      ...
      this.$validator.validateAll().then(valid => {
        if (valid) {
          this.$axios
            .post(
              "https://us-central1-richsnapp-174618.cloudfunctions.net/mail",
              {
                name: this.name,
                email: this.email,
