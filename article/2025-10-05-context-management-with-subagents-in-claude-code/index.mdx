...
in the case of agentic models, to stay aligned with a particular task. Some of the methods you may or may not have used:
...
1. **Lack of concurrency:** The subagents in Claude Code are not meant for concurrent execution. You might think that
   spinning off a subagent (sounds like a thread, right?) would allow for multiple subagents. However, while they do
   execute asynchronously, any commands sent while the subagent is working will be queued. If you want concurrency
   you'll need separate conversations using some sort of git worktrees + terminal multiplexer + note sharing through the
   filesystem workflow. _Note: this may not be true for subagents executed using the
   [Claude Agent SDK](https://docs.claude.com/en/api/agent-sdk/subagents)_
2. **Lack of insight:** While you can see and approve tool usage, its thinking and its general agentic loop will be
