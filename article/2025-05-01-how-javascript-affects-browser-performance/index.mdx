...
Let's look at another example, this one even breaks the work into smaller chunks but these chunks are still enough to
result in a poor frame rate. Try dragging the box below, then click `work()` and try again. See how the experience 
changes:
