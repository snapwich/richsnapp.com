...
Of particular note is my `.zprofile` with `.zprofile.d/` directory pattern which allows you to add various env files to
the container when started for the first time, such as anthropic keys.
...
# the first time you run nvim it will install all the plugins specified in ~/.config/nvim/lazy-lock.json.
nvim
# add necessary env files
nvim ~/.zprofile.d/anthropic-keys.sh
```
...
Host dev.home
  User dev
  Port 2222
  IdentityFile ~/.ssh/id_rsa # or whatever private key you associated with authorized_keys on the container
  LogLevel QUIET
...
<div>
<LightboxGallery selector="img" style="position: sticky; top: 10px; z-index: 999">
  <figure>
