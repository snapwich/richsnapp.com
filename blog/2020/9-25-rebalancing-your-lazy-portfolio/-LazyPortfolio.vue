      ...
      <template v-if="portfolios.length">
        <div class="row">
          <div class="col col-md-6">
            ...
            <select class="form-control" v-model="presetPortfolio" size="10">
              ...
              <optgroup v-for="group in portfolios" :label="group.text">
                <option v-for="portfolio in group.values" :value="portfolio.value">
                  {{ portfolio.text }}
                </option>
              </optgroup>
            </select>
          ...
          <div class="col col-md-6" v-if="hasData(assets)">
            <h3>Target Allocation<span v-if="totalFinalPercentages > 0"> vs. Final</span></h3>
          ...
          <div class="row">
            <div class="col-sm-7">
              <div class="input-group">
                <currency-input class="form-control contributionAmount" v-model="contributionAmount" :currency="currency.code" />
                <span class="input-group-btn">
                  <button class="btn btn-primary btn" type="button" @click="autoCalculate">Auto Calculate</button>
                </span>
              </div>
              <input type="range"
                     class="form-control-range slider"
                     :min="-contributionSliderMax"
                     :max="contributionSliderMax" step="10"
            ...
            <div class="col-sm-5 currency">
              <v-select class="currency-input" v-model="currency" :options="formattedCurrencies" :clearable="false"></v-select>
            </div>
          </div>
        </div>
      </div>
    ...
    <table class="table table-sm table-striped">
      <thead>
        <tr>
          ...
          <th class="buySellAmount">{{ contributionAmount >= 0 ? "Buy/Sell?" : "Sell/Buy?" }} <input type="checkbox"
                                                     title="Allow assets to be sold?"
                                                     v-model="canBuySellAll" /></th>
      ...
      <tbody name="assets" is="transition-group">
        <tr v-for="(asset, i) in assets" :key="i" class="asset">
            ...
            <v-select
              ...
              <template v-slot:list-header>
                <li class="list-header">
                  <button class="btn btn-sm btn-default" type="button" @click="removeAsset(asset)"><i class="fa fa-trash-o" /> Remove Asset</button>
                </li>
              </template>
            </v-select>
          </td>
          ...
          <td class="currentValue">
            <currency-input class="form-control input-sm" v-model="asset.value" :currency="currency.code" />
          </td>
          <td class="currentPercentage">
            {{ currentPercentages[i].toFixed(1) }}%
          </td>
          <td class="buySellAmount" :class="coloredNumberClasses(buySell[i])">
            {{ format(buySell[i]) }} <input type="checkbox"
      ...
      <tfoot>
        <tr>
          ...
          <td class="currentPercentage">{{ totalCurrentPercentages.value.toFixed(1) }}%</td>
          <td class="buySellAmount" :class="coloredNumberClasses(totalBuySell)">{{ format(totalBuySell) }}</td>
          <td class="finalValue">{{ format(totalFinalValues) }}</td>
          <td class="finalPercentage">{{ totalFinalPercentages.value.toFixed(1) }}%</td>
        </tr>
      </tfoot>
    </table>
    <footer>
      <button class="btn btn-primary add" @click="addAsset" type="button">Add Asset</button>
      <button class="btn expand-csv" @click="showCsv = !showCsv">
        <i :class="['fa fa-angle-down', {expanded: showCsv}]"></i> view tab-delimited
      </button>
      <textarea class="form-control csv" v-if="showCsv" readonly>{{ tabDelimited }}</textarea>
    </footer>
  </div>
</template>
...
<style lang="less" scoped>
  ...
  table {
    ...
    @media (max-width: @screen-md-max) {
      tr {
        ...
        .currentPercentage, .finalPercentage, .buySellAmount, .finalValue {
          width: 25%;
        }
      }
    }
    @media (max-width: @screen-xs-max) {
      tr {
        ...
        .finalPercentage, .buySellAmount, .finalValue {
          width: 33.3333%;
        }
      }
    }
  ...
  .assets-enter-active, .assets-leave-active {
    transition: background-color 2s;
  }

  .assets-enter, .assets-leave-to {
    transition: none;
